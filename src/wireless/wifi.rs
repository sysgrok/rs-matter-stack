    ...
    E: Embedding + 'static,
{
        ...
        U: UserTask,
    {
        ...
        let mut net_task =
            unwrap!(alloc.box_pin(self.run_wifi_with_memory(wifi, handler, user, alloc)));
...
impl<M, E, H, U> GattTask
    for MatterStackWirelessTaskWithMemory<'static, '_, M, wireless::Wifi, E, H, U>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
...
impl<M, E, H, X> WifiTask for MatterStackWirelessTaskWithMemory<'_, '_, M, wireless::Wifi, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
...
impl<M, E, H, X> WifiCoexTask
    for MatterStackWirelessTaskWithMemory<'static, '_, M, wireless::Wifi, E, H, X>
where
    M: RawMutex + Send + Sync + 'static,
    E: Embedding + 'static,
    H: AsyncMetadata + AsyncHandler,
