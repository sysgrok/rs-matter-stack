...
use rs_matter::dm::endpoints::{wifi_sys_handler, ROOT_ENDPOINT_ID};
#[cfg(not(feature = "icd"))]
    ...
    E: Embedding,
{
    ...
    /// Return a metadata for the root (Endpoint 0) of the Matter Node
    /// configured for BLE+Wifi network, with the ICD Management cluster
    /// included.
    #[cfg(feature = "icd")]
    pub const fn root_endpoint() -> Endpoint<'static> {
        const ENDPOINT: Endpoint<'static> = Endpoint {
            ...
            clusters: rs_matter::clusters!(wifi; IcdMgmtHandler::CLUSTER),
            client_clusters: &[],
            unique_id: None,
            semantic_tags: &[],
        };
...
impl<'a, const B: usize, E, C, H, K, U, Q> GattTask
    for MatterStackWirelessTask<'a, B, wireless::Wifi, E, C, H, K, U, Q>
where
    E: Embedding,
    C: Crypto,
    H: DataModel,
...
impl<'a, const B: usize, E, C, H, K, X, Z> WifiTask
    for MatterStackWirelessTask<'a, B, wireless::Wifi, E, C, H, K, X, Z>
where
    E: Embedding,
    C: Crypto,
    H: DataModel,
...
impl<'a, const B: usize, E, C, H, K, X, Z> WifiCoexTask
    for MatterStackWirelessTask<'a, B, wireless::Wifi, E, C, H, K, X, Z>
where
    E: Embedding,
    C: Crypto,
